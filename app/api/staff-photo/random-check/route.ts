...
export async function POST(request: Request) {
  ...
  try {
    ...
    const staffName = normalizeStaffName(parsed.staffName);
    if (!staffName) return Response.json({ ok: false, error: "Enter your name before completing the check." }, { status: 400 });
    ...
    await mutateStaffState((state) => {
      const row = { day_key: dayKey, check_key: assignment.key, question: assignment.question, result, issue_id: issueId, completed_at: new Date().toISOString(), staff_name: staffName };
