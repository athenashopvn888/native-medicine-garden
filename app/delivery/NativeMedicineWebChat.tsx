...
type Conversation = { id: string; messages: Message[]; idReviews?: Review[]; customerIntent?: CustomerIntent; customerNumberMasked?: string; phoneVersion?: number; readyConsentRequired?: boolean; readyConsentVersion?: number; intakeCycleId?: string | null; intakeCycleDate?: string | null; intakeCycleSequence?: number };
...
export default function NativeMedicineWebChat() {
  ...
  const [readyConsentChecked, setReadyConsentChecked] = useState(false);
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Native Medicine Garden chat could not be restored."));
  ...
  async function start(event: FormEvent) {
    ...
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/session`, {
        ...
        body: JSON.stringify({ storeId: "NM", customerName: intent === "NEW_CUSTOMER" ? name : "", phone, intent, message: firstMessage, workflowVersion: "READY_V1", smsConsent }),
      }));
  ...
  async function confirmReadyConsent(event: FormEvent) {
    event.preventDefault();
    if (!token || !conversation?.readyConsentRequired || !conversation.readyConsentVersion || !readyConsentChecked) return;
    setBusy(true);
    setNotice("");
    try {
      const data = await payload(await fetch(`${API_BASE}/api/web-chat/ready-consent`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ smsConsent: true, readyConsentVersion: conversation.readyConsentVersion }),
      }));
      setConversation(data.conversation);
      setReadyConsentChecked(false);
      setNotice("READY text consent saved. The dispatcher can now send READY when available.");
    } catch (error) { setNotice(error instanceof Error ? error.message : "READY text consent could not be saved."); }
    finally { setBusy(false); }
  }
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Native Medicine Garden Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Native Medicine Garden Web Chat">
      <header><div><strong>Native Medicine Garden Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
      ...
      {!token ? (!availability || availability.state !== "AVAILABLE" ? <div className="sod-chat-start sod-chat-paused">
        ...
        <div className="sod-chat-scroll">
          ...
          {conversation?.readyConsentRequired && <form className="sod-ready-consent-recovery" onSubmit={confirmReadyConsent}>
            <strong>Confirm READY text consent</strong>
            <p>Your chat and order history will stay here. Confirm below so the dispatcher can send the READY delivery link when available.</p>
            <label><input required type="checkbox" checked={readyConsentChecked} onChange={(event) => setReadyConsentChecked(event.target.checked)} /><span>I agree to receive one READY delivery-link text for this order. Message and data rates may apply.</span></label>
            <button type="submit" disabled={busy || !readyConsentChecked}>{busy ? "Saving…" : "CONFIRM READY TEXT CONSENT"}</button>
          </form>}
        ...
        {!conversation?.readyConsentRequired && <form className="sod-chat-composer" onSubmit={send}><textarea aria-label="Web Chat message" maxLength={1000} value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Write a message" /><button type="submit" disabled={busy || !message.trim()}>Send</button></form>}
      </>}
