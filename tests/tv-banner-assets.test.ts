...
test("neither TV route mounts a top menu banner", () => {
  for (const route of ["tv", "tv2"]) {
    ...
    assert.doesNotMatch(page, /menuBanner|menuBannerImage/);
    assert.doesNotMatch(styles, /\.menuBanner|\.menuBannerImage/);
  }
});
